...
- **Smart price calculator** — `components/sections/smart-calculator/`. A two-mode widget behind one stable `SmartPriceCalculator` export (prop `showChatMode?: boolean`, default true): `index.tsx` (orchestrator + segmented **Calculator | Ask AI** toggle), `calculator-mode.tsx` (structured estimator, mobile-first: scrolling inputs + a sticky live price — desktop side panel / mobile bottom bar + details sheet via `price-panel.tsx`/`price-breakdown.tsx`), `chat-mode.tsx` (natural-language AI that reuses the assistant pipeline), `estimate-logic.ts` (pure engine), `constants.ts` (option lists), plus `option-card.tsx`/`suburb-input.tsx`. **Pricing = built-in defaults + live DB override:** `estimate-logic.ts` holds a baked Perth pricing model and `calculateEstimate(form, catalog)` overrides the headline range when a catalog row matches the chosen service+issue (flagged "Live price" in the UI). The catalog comes from the CMS via a new same-origin proxy **`app/api/pricing/route.ts`** → `GET {CMS}/api/pricing-items` (anonymous, sanitised, `[]`-on-fail), fetched client-side and module-cached by `use-pricing-catalog.ts` (`lib/cms/pricing-client.ts` is the typed server helper). No backend changes; degrades to defaults when the CMS is down. The **Ask AI** mode and the floating widget share `components/sections/chat/use-assistant-chat.ts` (the conversation hook: messages/typing/`sendMessage` → `/api/chat`). Conversion CTAs (Book/Quote/Call) reuse `in-chat-booking.tsx`/`in-chat-quote.tsx`. Surfaced at the `app/calculator` route, via `components/sections/calculator-dialog.tsx`, and as the in-chat `calculator` overlay (which passes `showChatMode={false}`).

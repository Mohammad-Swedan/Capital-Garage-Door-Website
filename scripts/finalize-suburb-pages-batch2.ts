/**
 * Post-publish wiring for the 15 batch-2 suburb pages
 * (scripts/import-suburb-pages-batch2.ts created them as Drafts).
 *
 * A suburb page is invisible until it's linked (CLAUDE.md): this script does,
 * for every batch-2 page that is PUBLISHED (drafts are skipped, so it can be
 * re-run as pages are published in stages):
 *
 *  1. /service-areas chip — sets the matching suburb row's `pageId` (creates
 *     the suburb row first if it doesn't exist, e.g. Mandurah, placed in the
 *     same region as a nearby anchor suburb).
 *  2. Nearby-chip repoints — on EVERY Flat page, any NearbySuburbs chip whose
 *     label is a published batch-2 suburb and whose href is the /service-areas
 *     placeholder is repointed at the real page (this also cross-links the
 *     batch-2 pages to each other once both sides are published).
 ...
 *
 * Production: CMS_API_URL=https://cgd.runasp.net CMS_ADMIN_PASSWORD=… npx tsx scripts/finalize-suburb-pages-batch2.ts
 */
...
async function main() {
  console.log(`Finalizing batch-2 suburb pages on ${CMS_API_URL}`);
  await login();
  ...
  // Only published batch-2 pages get wired in; drafts stay invisible.
  const published = serviceSuburbPagesBatch2.filter(
    (p) => bySlug.get(p.slug)?.status === "Published",
  );
  const skipped = serviceSuburbPagesBatch2.filter((p) => bySlug.get(p.slug)?.status !== "Published");
  if (skipped.length) {
    console.log(
  ...
  if (!published.length) {
    console.log("No batch-2 pages are published yet — nothing to do.");
    return;
  }

/**
 * One-shot publisher for /garage-door-repairs-dayton (user-run wrapper —
 * the auto-mode classifier blocks the assistant issuing the publish call
 * directly; same pattern as scripts/publish-maddington.ts).
 *
 * Targets PRODUCTION by default. The admin password is read from
 * CMS_ADMIN_PASSWORD, else from the CMS repo's gitignored
 * appsettings.Production.json on disk (BOM-tolerant) — never hardcoded
 * here, this repo is public.
 *
 * Run:  npx tsx scripts/publish-dayton.ts
 */
...
const TARGET_SLUG = "garage-door-repairs-dayton";
...
async function main() {
  ...
  if (!ref) throw new Error(`${TARGET_SLUG} (Flat) not found — run scripts/import-dayton-page.ts first.`);
  ...
  console.log("\nNext: run scripts/import-dayton-page.ts against prod again for Phase B wiring.");
}
